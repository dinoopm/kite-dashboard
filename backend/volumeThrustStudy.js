...
function findEvents(bars, { mult = VOLUME_THRUST_MULT, window = CONFIRM_WINDOW } = {}) {
  ...
  for (let i = CROSS_SLOW + 1; i < bars.length; i++) {
    ...
    if (hit) {
      ...
      crosses.push({ index: i, date: bars[i].date, confirmed: at >= 0, barsAgo: at >= 0 ? i - at : null });
    }
      ...
      return {
        ...
        verdict: !ex.length ? 'no firings'
          ...
          : (t == null || Math.abs(t) < 2)
            ...
            : `${med >= 0 ? '+' : ''}${med.toFixed(2)}% median excess (t=${t}, n=${ex.length})`,
      };
    }),
  };
}
    ...
    const lift = HORIZONS.map(h => {
      ...
      return {
        ...
        verdict: (a.length < MIN_N || b.length < MIN_N)
          ...
          : (t == null || Math.abs(t) < 2)
            ? `no measurable difference (${diff >= 0 ? '+' : ''}${diff.toFixed(2)}pp, t=${t ?? '—'}) — the filter would discard firings for nothing`
            : `${diff >= 0 ? '+' : ''}${diff.toFixed(2)}pp for confirmed firings (t=${t})`,
      };
    });
...
module.exports = { runVolumeThrustStudy, findEvents, welchT, tAgainstZero, HORIZONS, MIN_N, BENCHMARK };
