...
const BLOCKED_SIGNALS = [
  ...
  },
  {
    ...
    source: 'recorded',
    blockedReason: 'A market-wide claim, not a per-symbol one, so signals/scorecard.js — which measures each symbol against an index — cannot express it, the same reason expiry_volatility sits here. It IS recorded before the outcome exists: dailyJobs writes macro_signal_snapshots every day, and on 2026-09-16 that day\'s snapshot was written at 09:10 UTC against an 18:00 UTC announcement, which is what makes the record checkable at all. It is now also SCORED, by backend/macro/fomc.js and backend/macro/fomcScorecard.js (GET /api/macro/fomc-scorecard): each meeting\'s decision is derived from DFEDTARU — unrevised, and effective the day after the announcement, so the value on the meeting date is deliberately not read — and paired with the bias that predates it. What it still is not is SCOREABLE, because the sample is the problem, not the machinery. A regime is a STATE that persists for months and this repo\'s first rule is that signals fire on transitions, not states, so the honest unit is the FOMC decision: eight a year, floor of 20, roughly two and a half years of live recording away. As of 2026-09-16 the tally is 0 resolved and 1 pending — the panel read hold-compatible into a 25bp hike, and one meeting is not evidence in either direction. A credible sample sooner needs the ALFRED vintage reconstruction (FRED_API_KEY + macro/ingest.js runIngest({ mode: "backfill", asOf }) walked back through history), which would give on the order of 60-90 transitions since 2000 using data as it was actually known at the time; that evidence would be reconstructed and must be reported separately rather than pooled with the recorded rows. Until the recorded sample clears the floor, treat the panel as a data display, not as evidence about anything.',
