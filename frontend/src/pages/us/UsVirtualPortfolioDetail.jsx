...
const fmtSignedMoney = (v) => (v == null ? '—' : `${v >= 0 ? '+' : '−'}$${Math.abs(v).toLocaleString('en-US', { maximumFractionDigits: 2 })}`);
const fmtPct = (v) => (v == null ? '—' : `${v > 0 ? '+' : ''}${v.toFixed(2)}%`);
const fmtQty = (v) => (v == null ? '—' : Number(v).toLocaleString('en-US', { maximumFractionDigits: 4 }));
...
export default function UsVirtualPortfolioDetail() {
  ...
  const baseRef = useRef([]); // canonical server holdings (no price fields)
  ...
  const fetchQuotes = async (syms, signal) => {
    if (!syms.length) return {};
    try {
      const qRes = await fetchWithAbort(`${API}/api/us/quotes`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ instruments: syms }), signal,
      });
      const qRaw = await qRes.json();
      return qRaw?.content?.[0]?.text ? JSON.parse(qRaw.content[0].text) : {};
    } catch (e) { if (e.name === 'AbortError') throw e; return {}; }
  };

  const loadHoldings = useCallback(async (signal) => {
    try {
      const res = await fetchWithAbort(`${API}/api/us/portfolios/${portfolioId}`, { signal });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to load portfolio');
      const p = data.portfolio;
      setPortfolio({ id: p.id, name: p.name });
      const hs = p.holdings || [];
      baseRef.current = hs;
      const quotes = await fetchQuotes(hs.map(h => h.symbol), signal);
      setHoldings(hs.map(h => {
        const q = quotes[h.symbol] || {};
        return { ...h, token: h.symbol, ltp: q.last_price ?? null, previousClose: q.ohlc?.close ?? null };
      }));
  ...
  // PATCH the whole holdings array to the server (source of truth).
  const persistHoldings = useCallback(async (nextBase) => {
    baseRef.current = nextBase;
    try {
      const res = await fetch(`${API}/api/us/portfolios/${portfolioId}`, {
  ...
  const refreshPrices = useCallback(async () => {
    setRefreshing(true);
    try {
      const quotes = await fetchQuotes(baseRef.current.map(h => h.symbol));
      setHoldings(prev => (prev || []).map(h => {
        const q = quotes[h.symbol];
        return q ? { ...h, ltp: q.last_price ?? null, previousClose: q.ohlc?.close ?? null } : h;
      }));
      setLastRefreshed(new Date());
      setError(null);
    } catch (e) {
  ...
  const addHolding = async () => {
    ...
    const newAvg = Number(avgCost) || 0, newQty = Number(qty) || 0;
    let merged = false;
    try {
      const base = baseRef.current;
      const existing = base.find(h => h.symbol === sym);
      let next;
      if (existing) {
        merged = true;
        const totQty = (Number(existing.quantity) || 0) + newQty;
        const blended = totQty > 0
          ? (((Number(existing.avgCost) || 0) * (Number(existing.quantity) || 0)) + newAvg * newQty) / totQty
          : newAvg;
        next = base.map(h => h.symbol === sym ? { ...h, quantity: totQty, avgCost: +blended.toFixed(4) } : h);
      } else {
        next = [...base, { id: uid(), symbol: sym, name: picked.name, avgCost: newAvg, quantity: newQty }];
      }
  ...
  const saveHolding = (id, patch) => {
    persistHoldings(baseRef.current.map(h => h.id === id ? { ...h, ...patch } : h));
  };

  const removeHolding = async (id, symbol) => {
    if (!window.confirm(`Remove ${symbol} from this portfolio?`)) return;
    await persistHoldings(baseRef.current.filter(h => h.id !== id));
    loadHoldings();
  };
  ...
  const deletePortfolio = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/us/portfolios/${portfolioId}`, { method: 'DELETE' });
  ...
  return (
    <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Link to="/us/virtual" style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', textDecoration: 'none' }}>← Back to Portfolios</Link>
        {portfolio && (
              ...
              {sorted.map(r => (
                <tr key={r.id}>
                  <td style={{ ...td, textAlign: 'left', cursor: r.token ? 'pointer' : 'default', width: '160px', maxWidth: '160px' }}
                    onClick={() => r.token && navigate(`/us/${encodeURIComponent(r.symbol)}`)}>
