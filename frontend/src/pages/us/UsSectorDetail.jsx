...
// ETF labels for the drilldown header (sectorKey is a plain ticker for US).
const US_LABELS = {
...
export default function UsSectorDetail() {
  ...
  const sectorName = US_LABELS[sectorKey] || sectorKey;
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        // Fetch sector + SPY quotes
        const qRes = await fetchWithAbort(`${API}/api/us/quotes`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ instruments: [sectorKey, 'SPY'] }),
          signal
        });
        ...
        // Parallel: fetch sector history + SPY history + constituents
        const [sectorHistRes, niftyHistRes, constRes] = await Promise.allSettled([
          sq?.instrument_token
            ? fetchWithAbort(`${API}/api/us/historical-full/${sq.instrument_token}`, { signal }).then(r => r.json())
            : Promise.resolve(null),
          niftyToken
            ? fetchWithAbort(`${API}/api/us/historical-full/${niftyToken}`, { signal }).then(r => r.json())
            : Promise.resolve(null),
          fetchWithAbort(`${API}/api/us/sector-constituents/${encodeURIComponent(sectorKey)}`, { signal }).then(r => r.json()),
        ]);
  ...
  const fetchRRG = useCallback(async (signal) => {
    ...
    try {
      ...
      const url = `${API}/api/us/rrg?benchmark=${encodeURIComponent(sectorKey)}&securities=${encodeURIComponent(securities)}`;
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
          ...
          <tbody>
            {sortedStocks.map((s, idx) => (
              <tr
                key={s.key}
                onClick={() => s.token && navigate(`/us/${encodeURIComponent(s.token)}`)}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                ...
                <th style={{ textAlign: 'right', padding: '0.5rem', color: 'var(--text-secondary)' }}>RS vs SPY</th>
  ...
  if (error) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center' }}>
        ...
        <button onClick={() => navigate('/us')} style={{ marginTop: '1rem', padding: '0.5rem 1rem', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>
          ← Back to Indices
        </button>
      </div>
    );
  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '1rem', fontSize: '0.82rem', color: 'var(--text-secondary)' }}>
        <Link to="/us" style={{ color: 'var(--accent)', textDecoration: 'none' }}>Indices Performance</Link>
