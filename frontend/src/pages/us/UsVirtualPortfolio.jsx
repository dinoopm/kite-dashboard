...
// US Virtual Portfolios — paper portfolios of US stocks (persisted in Supabase).
export default function UsVirtualPortfolio() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(`${API}/api/us/portfolios`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to load portfolios');
      setPortfolios(data.portfolios || []);
      setError(null);
    } catch (e) { setError(e.message); setPortfolios([]); }
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const createPortfolio = async (e) => {
    ...
    setCreating(true);
    try {
      const res = await fetch(`${API}/api/us/portfolios`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name }) });
      const data = await res.json();
  ...
  const deletePortfolio = async (id, name) => {
    ...
    try {
      const res = await fetch(`${API}/api/us/portfolios/${id}`, { method: 'DELETE' });
  ...
  const renamePortfolio = async (id) => {
    ...
    try {
      const res = await fetch(`${API}/api/us/portfolios/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name }) });
  ...
  return (
    <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ margin: 0 }}>US Virtual Portfolios</h1>
          <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
            Build paper portfolios of US stocks — enter instrument, average cost and quantity; invested, current value,
            P&amp;L, day change and allocation are calculated live from market prices.
          </span>
        </div>
        <form onSubmit={createPortfolio} style={{ display: 'flex', gap: '0.5rem' }}>
          <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="New portfolio name…" maxLength={60}
                ...
                {isEditing ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                    <input autoFocus value={draft} onChange={e => setDraft(e.target.value)}
                      onKeyDown={e => { if (e.key === 'Enter') renamePortfolio(p.id); if (e.key === 'Escape') cancelEdit(); }} maxLength={60}
                  ...
                  <>
                    <Link to={`/us/virtual/${p.id}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
                      ...
                      <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                        {(p.holdings || []).length} {(p.holdings || []).length === 1 ? 'holding' : 'holdings'}
                      </div>
                    </Link>
                    <div style={{ position: 'absolute', top: '0.75rem', right: '0.75rem', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                      <button onClick={() => startEdit(p)} title="Rename portfolio" style={{ background: 'transparent', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: '0.85rem', lineHeight: 1 }}>✎</button>
                      <button onClick={() => deletePortfolio(p.id, p.name)} title="Delete portfolio" style={{ background: 'transparent', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: '1rem', lineHeight: 1 }}>✕</button>
                    </div>
                  </>
                )}
              </div>
