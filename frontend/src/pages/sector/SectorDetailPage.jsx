...
// ─── Main page ───────────────────────────────────────────────────────
export default function SectorDetailPage({ market }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        // Fetch sector + benchmark quotes
        const qRes = await fetchWithAbort(`${API}/api${market.apiPrefix}/quotes`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ instruments: [sectorKey, market.benchmarkKey] }),
          signal
        });
        const qRaw = await qRes.json();
        const quotes = qRaw?.content?.[0]?.text ? JSON.parse(qRaw.content[0].text) : {};
        const sq = quotes[sectorKey];
        const nq = quotes[market.benchmarkKey];
        ...
        const benchmarkToken = nq?.instrument_token ? String(nq.instrument_token) : null;

        // Parallel: fetch sector history + benchmark history + constituents
        const [sectorHistRes, benchmarkHistRes, constRes] = await Promise.allSettled([
          sq?.instrument_token
            ? fetchWithAbort(`${API}/api${market.apiPrefix}/historical-full/${sq.instrument_token}`, { signal }).then(r => r.json())
            : Promise.resolve(null),
          benchmarkToken
            ? fetchWithAbort(`${API}/api${market.apiPrefix}/historical-full/${benchmarkToken}`, { signal }).then(r => r.json())
            : Promise.resolve(null),
          fetchWithAbort(`${API}/api${market.apiPrefix}/sector-constituents/${encodeURIComponent(sectorKey)}`, { signal }).then(r => r.json()),
        ]);
        ...
        // benchmark history → returns only
        const niftyArr = benchmarkHistRes.status === 'fulfilled' ? parseHistPayload(benchmarkHistRes.value) : null;
        if (niftyArr?.length) {
          const sorted = [...niftyArr].sort((a, b) => a.date.localeCompare(b.date));
          const price = nq?.last_price ?? sorted[sorted.length - 1]?.close ?? 0;
          setBenchmarkReturns(calculateHistoricalReturns(sorted, price));
        }
  ...
  useEffect(() => {
    ...
    async function loadStocks() {
      ...
      for (const c of constituents) {
        ...
        let hitCache = true;
        try {
          const hRes = await fetchWithAbort(`${API}/api${market.apiPrefix}/historical-full/${c.token}`, { signal });
  ...
  const enrichedStockData = useMemo(() => {
    ...
    return withRaw.map(s => {
      ...
      return {
        ...
        rsVsNifty: s['1M'] != null && benchmarkReturns?.['1M'] != null ? s['1M'] - benchmarkReturns['1M'] : null,
      };
    });
  }, [stockData, constituents.length, sectorReturns, benchmarkReturns]);
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
          ...
          <tbody>
            {sortedStocks.map((s, idx) => (
              <tr
                key={s.key}
                onClick={() => s.token && navigate(market.instrumentHref(s))}
              ...
              >
                ...
                <td style={{ padding: '0.4rem', textAlign: 'right', fontWeight: 600 }}>
                  {s.price ? `${market.currency}${s.price.toLocaleString(market.locale, { maximumFractionDigits: 2 })}` : '–'}
                </td>
                <Cell value={s['1D']} />
                <Cell value={s['1W']} />
                <Cell value={s['1M']} />
  ...
  useEffect(() => {
    ...
    const pull = async () => {
      try {
        ...
        const res = await fetchWithAbort(`${API}/api${market.apiPrefix}/sector-alerts/${encodeURIComponent(sectorKey)}`, { signal, timeoutMs: 120_000 });
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                ...
                <th style={{ textAlign: 'right', padding: '0.5rem', color: 'var(--text-secondary)' }}>RS vs {market.benchmarkLabel}</th>
  ...
  if (error) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center' }}>
        ...
        <button onClick={() => navigate(market.indicesRoute)} style={{ marginTop: '1rem', padding: '0.5rem 1rem', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>
          ← Back to Indices
        </button>
      </div>
    );
  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '1rem', fontSize: '0.82rem', color: 'var(--text-secondary)' }}>
        <Link to={market.indicesRoute} style={{ color: 'var(--accent)', textDecoration: 'none' }}>Indices Performance</Link>
          ...
          {sectorQuote && (
            <>
              <span style={{ fontSize: '1.25rem', fontWeight: 700 }}>
                {market.currency}{sectorQuote.price.toLocaleString(market.locale, { maximumFractionDigits: 2 })}
              </span>
      ...
      <div style={{ paddingTop: '1rem' }}>
        ...
        {market.hasAlertsTab && activeTab === 'alerts' && renderAlerts()}
      </div>
